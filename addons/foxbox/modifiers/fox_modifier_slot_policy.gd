...
## Handles "slots" in a FoxModifierManager 
...
## How many FoxModifierInstances this SlotPolicy can have.
...
func try_add(mod: FoxModifier, target: Node) -> bool:
	...
	if not node: 
		push_error("ERROR: a FoxModifierInstance couldn't be found\
		under try_add() in FoxModifierSlotPolicy ",get_path())
		return false
