...
signal occupant_sat(occupant : Node, seat : FoxSeat)
signal occupant_left(occupant : Node, seat : FoxSeat)
signal occupant_changed(occupant : Node, seat : FoxSeat)
...
func _ready() -> void:
	...
	if marker.get("global_position") == null:
		printerr("ERROR: FoxSeat.marker doesn't have a global_position. ",get_path())
	if marker.get("global_rotation") == null:
		printerr("ERROR: FoxSeat.marker doesn't have a global_rotation. ",get_path())
...
func sit(new_occupant: Node) -> void:
	if occupant != null:
		printerr("ERROR: FoxSeat already has an occupant but \
