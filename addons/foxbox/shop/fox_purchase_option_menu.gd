...
extends FoxControl
class_name FoxPurchaseOptionMenu
...
signal slot_pressed(option : FoxPurchaseOption)
...
## Updates the FoxPurchaseOptionSlot's based on
...
func update_affordability(_current_funds: int = current_funds) -> void:
	for slot in container.get_children():
		if slot is FoxPurchaseOptionSlot:
			var can_buy = FoxShopService.can_purchase(slot.data, _current_funds)
...
func add_option(option: FoxPurchaseOption, override_cost: float = NAN) -> void:
	var slot = slot_scene.instantiate() as FoxPurchaseOptionSlot
...
func populate(catalog: FoxPurchaseCatalog, override_costs: Array = []) -> void:
	clear_all_options()
...
func _on_slot_pressed(option: FoxPurchaseOption) -> void:
