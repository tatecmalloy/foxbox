...
class ModifierInstance:
	var modifier: TateModifier
	var time_left: float
	
	func _init(mod: TateModifier):
		modifier = mod
...
func add_modifier(mod: TateModifier, target: Node) -> void:
	# 1. UNIQUE: Refresh timer, don't re-run code
	if mod.stack_mode == TateModifier.StackMode.UNIQUE and active_modifiers.has(mod.modifier_id):
		active_modifiers[mod.modifier_id].time_left = mod.duration
		return

	# 2. ADDITIVE: Call a special function on the existing mod
	if mod.stack_mode == TateModifier.StackMode.ADDITIVE and active_modifiers.has(mod.modifier_id):
	...
	var key = mod.modifier_id if mod.stack_mode == TateModifier.StackMode.UNIQUE else str(mod.modifier_id, "_", Time.get_ticks_msec())
