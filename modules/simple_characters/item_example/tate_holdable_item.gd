extends TateNode3D
class_name TateHoldableItem
## An item that can be held by a TateCharacter.
...
func _ready() -> void:
	if off_hand_target and main_hand_target == null:
		push_warning("WARNING: TateHoldableItem has a off_hand_target but no main_hand_target, ",get_path())
	if off_elbow_pole and main_elbow_pole == null:
		push_warning("WARNING: TateHoldableItem has a off_elbow_pole but no main_elbow_pole, ",get_path())
