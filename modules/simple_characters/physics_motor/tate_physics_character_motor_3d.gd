...
#var _captured_platform_velocity := Vector3.ZERO
...
func _physics_process(delta: float) -> void:
	...
	"""# 1. Get the ground beneath your feet
	if floor_cast.is_colliding():
		var ground = floor_cast.get_collider()

		# 2. If it's a moving object, grab its velocity
		if ground is RigidBody3D:
			var platform_v = ground.linear_velocity

			# 3. Add that velocity to your own movement intent
			# This 'glues' you to the platform's current motion
			var move_intent = (rigid_body.global_transform.basis * Vector3(input_direction.x, 0, input_direction.y)) * target_speed

			rigid_body.linear_velocity.x = move_intent.x + platform_v.x
			rigid_body.linear_velocity.z = move_intent.z + platform_v.z"""
...
func _movement(delta):
	...
	var move_direction : Vector3 = (forward * input_direction.y + right * input_direction.x).normalized() * input_strength
	
	var ground_velocity := _get_ground_velocity()
	var target_velocity : Vector3 = move_direction * target_speed + ground_velocity
	
	var velocity_error := target_velocity - rigid_body.linear_velocity
	var correction_impulse := _pid.update(velocity_error, delta) * acceleration
	
	
	correction_impulse.y = 0.0
	
	rigid_body.apply_central_impulse(correction_impulse + ground_velocity)
"""
...
func jump():
	...
	var _captured_platform_velocity = _get_ground_velocity() * rigid_body.mass
	print(_captured_platform_velocity)
	_captured_platform_velocity.y = 0.0
	
	if not has_jumped:
		#rigid_body.apply_central_impulse(Vector3.UP * jump_force + _captured_platform_velocity)
		#rigid_body.linear_velocity.y = 0.0
		rigid_body.apply_central_impulse(Vector3.UP * 600.0)
		has_jumped = true
